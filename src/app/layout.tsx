import type { Metadata } from "next";
import { Geist, Geist_Mono } from "next/font/google";
import "./globals.css";
import Navbar from "@/components/Navbar/Navbar";
import { AuthProvider } from "@/context/AuthContext";
import { FavoriteProvider } from "@/context/FavoriteContext";
import { Toaster } from "@/components/ui/sonner";
import Footer from "@/components/Footer/Footer";

const geistSans = Geist({
  variable: "--font-geist-sans",
  subsets: ["latin"],
});

const geistMono = Geist_Mono({
  variable: "--font-geist-mono",
  subsets: ["latin"],
});

export const metadata: Metadata = {
  title: "Egypt Voyage",
  description: "Explore the best hotels, restaurants, landmarks, and travel programs in Egypt. Plan your perfect trip with Egypt Voyage easily",
  keywords: [
    "Egypt travel",
    "hotels in Egypt",
    "restaurants in Egypt",
    "tourism Egypt",
    "travel programs Egypt"
  ],
};

export default function RootLayout({
  children,
}: Readonly<{
  children: React.ReactNode;
}>) {
  return (
    <html lang="en">
      <body className={`${geistSans.variable} ${geistMono.variable} antialiased min-h-screen bg-linear-to-b from-[#F8DA9A] to-[#0D3B66]`}>
        <AuthProvider>
          <FavoriteProvider>
            <Navbar />
            {children}
            <Footer />
            <Toaster theme="system" richColors position="top-right" />
          </FavoriteProvider>
        </AuthProvider>
      </body>
    </html>
  );
}
