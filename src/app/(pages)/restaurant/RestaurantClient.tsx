...
export default function RestaurantClient({ restaurants }: { restaurants: RestaurantI[] }) {
    ...
    return <>
        ...
        <div className='container mx-auto p-2 lg:p-1.5 xl:p-2 md:p-2 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 items-stretch'>
            {filteredRestaurant.map((restaurant) => {
                ...
                return (
                    <div key={restaurant.id} className='p-0.5'>
                        <Card className="flex flex-col transform transition-all duration-500 hover:shadow-[0_0_13px_rgba(0,0,0,0.15)] hover:shadow-black">
                            <Link href={'/restaurant/' + restaurant.id}>
                                <div className="relative w-full aspect-4/3">
                                    <Image src={restaurant.imageCover} alt={restaurant.restaurantName} fill sizes="(max-width: 640px) 100vw, (max-width: 1024px) 50vw, (max-width: 1280px) 33vw, 25vw" className="object-cover rounded-t-xl" />
                                </div>
                                <CardHeader>
                                    <CardTitle className="mt-2.5">{restaurant.restaurantName}</CardTitle>
                                </CardHeader>
                                <div className='flex justify-between items-center p-1.5 mt-1.5'>
                                    <CardAction className='justify-self-start'>{restaurant.location.city}</CardAction>
                                    <div className='flex'>
                                        {Array.from({ length: restaurant.rating }, (_, index) => (
                                            <StarIcon key={index} className='w-5 h-5 mt-1 fill-yellow-300 text-yellow-300' />
                                        ))}
                                        <div className='ms-1.5 text-lg text-yellow-300'>({restaurant.rating})</div>
                                    </div>
                                </div>
                            </Link>
                            <Button className={`mt-2.5 cursor-pointer mx-7 py-5 mb-5 bg-linear-to-b transition-all duration-300 from-[#D3A15C] to-[#00000055] text-white`} onClick={() => toggleFavorite(restaurant.id, "restaurant")} disabled={loading}>
